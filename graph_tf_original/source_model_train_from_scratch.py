...
# for source_task_name in source_task_names.columns:
# import pdb; pdb.set_trace()
source_task_name = "task3"

task_index = 3
print(len(source_task_names[source_task_name].values))

# 在每次运行时生成唯一的日志目录名称
log_dir = "logs/souce_{}_rs50_lr=1e-3".format(task_index) + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
...
early_stop_counter = EarlyStopCounter(threshold=50)
...
for epoch in range(1000):
    ...
    print("train_acc:",np.mean(train_epoch_acc))
    ...
    writer.add_scalar('source_test_acc'.format(task_index), np.mean(epoch_acc),epoch)
    writer.add_scalar('source_train_acc'.format(task_index), np.mean(train_epoch_acc),epoch)
    # 判断是否停止训练
    ...
    torch.save(state_dict, './save/50lr=1e-3_new/source_{}_resnet_50_without_pretrained.pth'.format(task_index))
    print("source_task:", task_index)
